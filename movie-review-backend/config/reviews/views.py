...
def toggle_review_like(request, pk):
    ...
    if review.likes.filter(user=request.user).exists():
        review.likes.filter(user=request.user).delete()
        return Response({'status': 'unliked', 'likes_count': review.likes.count(), 'is_liked': False})
    else:
        # Import ReviewLike here to avoid circular imports if any, or just use related manager create
        # But we added ReviewLike model, so we can use it. 
        # Actually simpler to use the related manager if we can, but ReviewLike is a separate model.
        # Let's import ReviewLike at top or locally.
        from .models import ReviewLike
        ReviewLike.objects.create(user=request.user, review=review)
        return Response({'status': 'liked', 'likes_count': review.likes.count(), 'is_liked': True})

class MovieList(generics.ListCreateAPIView):
    queryset = Movie.objects.all()
    serializer_class = MovieSerializer
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = ['genre', 'type']
